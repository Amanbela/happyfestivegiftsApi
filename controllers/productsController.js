...
exports.create = (req, res) => {
  ...
  const newProd = { id: nextId++, name, price: price || 0, description: description || '' };
...
exports._reset = () => { products = []; nextId = 1; };
...
const scrapeFlipkart = async (search, highprice) => {
  ...
  $('div.DOjaWF.YJG4Cf div:nth-child(1)').each((i, element) => {
    ...
    if (title && price) {
      const exists = results.find(p => p.title === title && p.price === price);
      if (!exists) {
        results.push({
          image,
          title,
...
const scrapeAmazon = async (search, category, highprice) => {
  ...
  $('.s-widget-container').each((i, element) => {
    ...
    const offprice = $(element).find('div.a-section.aok-inline-block > span').children('.a-offscreen').text();
...
exports.scrape = async (req, res) => {
  ...
  try {
    ...
    merged = merged.sort((p1, p2) => parseFloat(p1.price || 0) - parseFloat(p2.price || 0));
    res.json(merged);
  } catch (err) {
